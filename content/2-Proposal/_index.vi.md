...
## 4.2 Yêu cầu chuẩn bị (Prerequisites)

### Tài nguyên và công cụ

- AWS Account có quyền sử dụng Amazon EC2, Amazon VPC, IAM, Amazon CloudTrail, Amazon GuardDuty, AWS Security Hub, AWS Config, IAM Access Analyzer, Amazon EventBridge, AWS Step Functions, AWS Lambda, Amazon SNS, Amazon CloudWatch, Amazon S3, AWS KMS, Amazon Athena, AWS WAF và Amazon CloudFront.
...
## 6.1 Môi trường Workshop / Hướng dânx
...
### Lưu ý tối ưu chi phí Workshop

Chi phí trên giả định toàn bộ hệ thống chạy liên tục. Trong môi trường học tập có thể giảm đáng kể bằng cách:
...
Chi phí Production không thể xác định chính xác chỉ dựa trên số lượng dịch vụ. Giá thực tế phụ thuộc vào:
...
## 7.2 Kế hoạch dự phòng (Contingency Plan)

### Trường hợp phát hiện Security Finding nhưng hệ thống phản ứng tự động thất bại

Khi GuardDuty hoặc Security Hub phát hiện sự cố nhưng EventBridge, Step Functions hoặc Lambda không thực hiện được hành động phản ứng, quản trị viên sẽ kiểm tra:
...
### Trường hợp EC2 bị xâm nhập hoặc có hành vi bất thường

Khi GuardDuty phát hiện dấu hiệu như Port Scanning, Credential Compromise hoặc hoạt động đáng ngờ:
...
### Trường hợp dữ liệu log bị mất hoặc không thể truy vấn

Nếu Amazon S3, CloudTrail hoặc Athena gặp lỗi:
...
### Trường hợp Lambda xử lý sai Security Finding

Trong trường hợp Lambda thực hiện hành động nhầm đối với tài nguyên hợp lệ:
...
### Trường hợp chi phí AWS vượt dự kiến

Do môi trường Workshop sử dụng nhiều dịch vụ có chi phí cố định như Application Load Balancer, NAT Gateway, AWS Config và WAF, chi phí có thể phát sinh cao hơn khi để hệ thống chạy liên tục.
...
### Trường hợp cần phục hồi toàn bộ hệ thống

Đối với môi trường Production, có thể triển khai các phương án mở rộng:
